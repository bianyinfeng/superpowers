...
(function normalizeMockData(data) {
  ...
  var currentUserName = data.currentUser && data.currentUser.name ? data.currentUser.name : '当前用户';
  var envFlow = ['DEV', 'SIT', 'UAT', 'GRAY', 'PRD'];
  ...
  function normalizedScriptStatus(value) {
    var text = String(value || '');
    if (/部署|通过|上线|已发布/.test(text)) {
      return '已复核';
    }
    if (/复核|审核/.test(text)) {
      return '待复核';
  ...
  function normalizedDeploymentStatus(value) {
    var text = String(value || '');
    if (/回滚/.test(text)) {
      return '回滚中';
    }
    if (/正式上线|已上线|上线/.test(text)) {
      return '已上线';
    }
    if (/灰度/.test(text)) {
      return '灰度中';
  ...
  function buildChecklist(allPassed) {
    return [
      { code: 'naming', title: '命名规范检查', description: 'table / column naming follows bank standards', checked: allPassed, note: allPassed ? '已通过' : '' },
      { code: 'auth', title: '数据权限检查', description: 'accessing authorized data sources only', checked: allPassed, note: allPassed ? '已通过' : '' },
      { code: 'performance', title: '性能评估', description: 'estimated run time and resource usage', checked: allPassed, note: allPassed ? '已通过' : '' },
      { code: 'exception', title: '异常处理', description: 'error handling present', checked: allPassed, note: allPassed ? '已通过' : '' },
      { code: 'quality', title: '数据质量埋点', description: 'quality checkpoints embedded', checked: allPassed, note: allPassed ? '已通过' : '' },
      { code: 'comment', title: '注释完整性', description: 'all major logic blocks commented', checked: allPassed, note: allPassed ? '已通过' : '' },
      { code: 'logging', title: '日志记录', description: 'logging statements present', checked: allPassed, note: allPassed ? '已通过' : '' },
      { code: 'idempotent', title: '幂等性验证', description: 'script is idempotent', checked: allPassed, note: allPassed ? '已通过' : '' }
    ];
  }

  function parseSpecColumns(ddl) {
    return String(ddl || '')
      .split('\n')
      .map(function (line) {
        var match = line.match(/^\s*([a-zA-Z_][a-zA-Z0-9_]*)\s+([^\s,]+(?:\([^)]*\))?)(?:\s+[^,]*)?COMMENT\s+'([^']+)'/);
        if (!match) {
          return null;
        }
        return {
  ...
  var requirementsById = {};
  data.requirements = safeArray(data.requirements).map(function (item, index) {
    var next = clone(item);
    next.id = next.id || next.reqNo || ('REQ-' + pad2(index + 1));
    next.name = next.name || next.title || next.reqNo || next.id;
    next.requirementName = next.requirementName || next.name;
    next.dueDate = next.dueDate || next.deadline || '';
    next.deadline = next.deadline || next.dueDate || '';
    requirementsById[next.id] = next;
    return next;
  });

  var workflowStepsByReq = {};
  safeArray(data.workflowSteps).forEach(function (step) {
    var reqId = step.reqId;
    if (!reqId) {
      return;
    }
    if (!workflowStepsByReq[reqId]) {
      workflowStepsByReq[reqId] = [];
    }
    workflowStepsByReq[reqId].push(clone(step));
  });

  var stepOrder = { business: 1, data: 2, tech: 3, compliance: 4 };
  data.workflows = data.requirements.map(function (req) {
    var steps = safeArray(workflowStepsByReq[req.id]).sort(function (a, b) {
      return (stepOrder[a.stepType] || 99) - (stepOrder[b.stepType] || 99);
    });
    var pendingStep = steps.find(function (step) { return step.status === 'pending'; }) || null;
    var rejectedStep = steps.find(function (step) { return step.status === 'rejected'; }) || null;
    var handledByMe = steps.some(function (step) {
      return step.approver === currentUserName && /approved|rejected/.test(step.status || '');
    });
    var allApproved = steps.length > 0 && steps.every(function (step) { return step.status === 'approved'; });
    var flowStatus = rejectedStep ? '已驳回' : (allApproved && /上线|完成/.test(req.status || '') ? '已上线' : '审核中');
    var currentStage = pendingStep ? pendingStep.stepName : (rejectedStep ? rejectedStep.stepName : (req.currentStep || req.currentStage || '流程结束'));
    ...
    return {
      ...
      currentReviewer: pendingStep ? pendingStep.approver : '',
      status: flowStatus,
      submitDate: req.submitDate,
      updatedAt: pendingStep ? pendingStep.timestamp : req.submitDate,
      mine: !!(pendingStep && pendingStep.approver === currentUserName),
      handledByMe: handledByMe,
      timeline: steps.map(function (step) {
        return {
          stage: step.stepName,
          action: step.status === 'approved' ? '审批通过' : step.status === 'rejected' ? '驳回' : '待处理',
          actor: step.approver,
          time: step.timestamp,
          comment: step.comments,
          state: step.status === 'approved' ? 'done' : step.status === 'rejected' ? 'rejected' : 'active'
        };
      })
    };
  });

  data.standards = safeArray(data.dataStandards).map(function (item, index) {
    return {
      code: item.stdCode,
      cnName: item.chineseName,
      enName: item.fieldName,
      dataType: item.dataType,
      length: String(item.maxLength || ''),
      ...
      complianceRefs: [item.regulatoryRef || '待补充监管引用'],
      versionHistory: [{ version: 'v1.0', date: '2024-01-01', summary: '同步演示标准库字段定义' }],
      usedTables: 8 + (index % 7) * 3,
      usedProjects: 3 + (index % 5)
    };
  });

  var standardLookup = {};
  data.standards.forEach(function (item) {
    standardLookup[item.code] = item;
  });

  data.standardMatches = safeArray(data.matchResults).map(function (item) {
    var standard = standardLookup[item.matchedStdCode] || null;
    return {
      id: item.id,
      requirementId: item.reqId,
      sourceField: item.sourceField,
      sourceDescription: item.sourceField,
      standardCode: item.matchedStdCode || '',
      standardName: standard ? standard.cnName : (item.matchedFieldName || ''),
      dataType: standard ? (standard.dataType + (standard.length ? '(' + standard.length + ')' : '')) : '',
      matchType: item.matchMethod,
      confidence: Number(item.confidence || 0),
      status: item.status === '待确认' ? '待确认' : item.status === '已调整' ? '需调整' : '已确认'
    };
  });

  data.specs = safeArray(data.devSpecs).map(function (item) {
    var req = requirementsById[item.reqId] || {};
    var reviewText = item.reviewComments || '';
    return {
      id: item.id,
      number: item.specNo,
      title: item.title,
      version: /^v/i.test(item.version || '') ? item.version : ('v' + item.version),
      status: /发布/.test(item.status || '') ? '已发布' : /审核|复核/.test(item.status || '') ? '待复核' : '编制中',
      requirementId: item.reqId,
      author: item.author,
      createdAt: item.createDate,
      updatedAt: item.createDate,
      basicInfo: {
        targetUse: req.targetPurpose || req.targetTable || '数据建模与交付',
        domain: req.businessUnit || '数据中台',
        scope: 'ODS → DWD/DWS/ADS',
        businessDescription: req.description || item.transformLogic || '暂无业务描述',
        lineageDescription: item.dataLineage || '待补充血缘说明'
      },
      ddl: item.tableStructure,
      columns: parseSpecColumns(item.tableStructure),
      transformations: [{ sourceTable: item.dataLineage || '待补充', rule: item.transformLogic || '待补充', targetField: req.targetTable || '目标表' }],
      businessRules: clone(item.qualityRules || []),
      qualityRules: safeArray(item.qualityRules).map(function (rule, index) {
        return {
          code: 'QR-AUTO-' + pad2(index + 1),
          description: rule,
          level: /唯一|非空/.test(rule) ? '阻断' : '预警',
          threshold: /唯一|非空/.test(rule) ? '异常率 = 0' : '异常率 < 1%'
        };
      }),
      versionHistory: [{ version: /^v/i.test(item.version || '') ? item.version : ('v' + item.version), date: item.createDate, author: item.author, summary: reviewText || '初始化开发规范' }],
      reviews: [{ reviewer: '系统导入', date: item.createDate, comment: reviewText || '待补充复核意见' }]
    };
  });

  var specNumberById = {};
  data.specs.forEach(function (item) {
    specNumberById[item.id] = item.number;
  });

  data.scripts = safeArray(data.scripts).map(function (item) {
    var status = normalizedScriptStatus(item.status);
    var parseTables = safeArray(item.parseResult && item.parseResult.tables);
    var parseOps = safeArray(item.parseResult && item.parseResult.operations);
    var versionHistory = safeArray(item.versionHistory).map(function (history) {
      return {
        version: history.v || history.version,
        author: history.author || item.author,
        date: history.date || item.createDate,
        summary: history.change || history.summary || '版本更新',
        code: item.content
      };
    });
    if (!versionHistory.length) {
      versionHistory.push({ version: item.version, author: item.author, date: item.createDate, summary: '初始版本', code: item.content });
    }
    return {
      id: item.id,
      ...
      specRef: specNumberById[item.specId] || item.specId,
      code: item.content,
      content: item.content,
      parse: {
        status: status === '已复核' ? '成功' : '警告',
        summary: safeArray(item.parseResult && item.parseResult.dependencies).length ? item.parseResult.dependencies.join('；') : '解析完成，待补充字段级血缘与风险说明。',
        sourceTables: parseTables.map(function (tableName, index) {
          var parts = String(tableName).split('.');
          return {
            tableName: tableName,
            database: parts.length > 1 ? parts[0] : 'default',
            operation: parseOps[index] || (index === parseTables.length - 1 ? 'WRITE' : 'READ'),
            rowEstimate: index === parseTables.length - 1 ? '目标表输出' : '待评估'
          };
        }),
        lineage: [{ source: parseTables[0] || '待分析', transform: '根据脚本内容自动解析', target: parseTables[parseTables.length - 1] || '待分析' }],
        metrics: {
          tableCount: parseTables.length,
          joinCount: Math.max(0, (String(item.content || '').match(/\bjoin\b/ig) || []).length),
          subqueryDepth: Math.max(0, (String(item.content || '').match(/\bselect\b/ig) || []).length - 1),
          planNotes: item.reviewComments || '待补充执行计划建议'
        },
        issues: safeArray(item.parseResult && item.parseResult.dependencies).length ? clone(item.parseResult.dependencies) : ['当前脚本缺少更细粒度的解析结果。']
      },
      reviewChecklist: buildChecklist(status === '已复核'),
  ...
  data.scripts.forEach(function (item) {
    scriptsById[item.id] = item;
  });

  data.deployments = safeArray(data.deployments).map(function (item, index) {
    var req = requirementsById[item.reqId] || {};
    var script = scriptsById[item.scriptId] || {};
    var envIndex = Math.max(0, envFlow.indexOf(item.currentEnv));
    var status = normalizedDeploymentStatus(item.status);
    var progress = status === '已上线' ? 100 : item.currentEnv === 'GRAY' ? Math.min(98, 60 + Math.round((item.grayPercent || 0) * 0.35)) : (envIndex + 1) * 20;
    return {
      id: item.id,
      name: item.name,
      requirementId: item.reqId,
      requirementName: req.name || req.title || item.reqId,
      scriptId: item.scriptId,
      scriptName: script.name || script.scriptName || item.scriptId,
      currentEnv: item.currentEnv,
      status: status,
      progress: progress,
      operator: item.deployer || currentUserName,
      updatedAt: formatDateTime(item.createDate),
      readyToAdvance: item.currentEnv !== 'PRD' && status !== '回滚中',
      testSummary: '通过 ' + (item.testResults && item.testResults.passed || 0) + ' 项，失败 ' + (item.testResults && item.testResults.failed || 0) + ' 项。',
      testReport: [
        { item: '测试通过', result: (item.testResults && item.testResults.passed || 0) > 0 ? '通过' : '待确认', detail: '通过 ' + (item.testResults && item.testResults.passed || 0) + ' 项' },
        { item: '测试失败', result: (item.testResults && item.testResults.failed || 0) > 0 ? '待确认' : '通过', detail: '失败 ' + (item.testResults && item.testResults.failed || 0) + ' 项' },
        { item: '灰度比例', result: status === '灰度中' ? '处理中' : '通过', detail: (item.grayPercent || 0) + '%' }
      ],
      steps: ['脚本打包', '环境验证', '验收测试', '灰度放量', '生产切换']
    };
  });

  data.deploymentEnvironments = envFlow.map(function (env, index) {
    return {
      name: env,
      health: env === 'UAT' || env === 'GRAY' ? 'warning' : 'healthy',
      database: env + '-BANK-CLUSTER',
      servers: 4 + index * 2,
      lastCheck: '09:' + pad2(58 - index * 3) + ':00',
      cpu: 35 + index * 7,
      memory: 48 + index * 5,
      deployments: data.deployments.filter(function (item) { return item.currentEnv === env; }).map(function (item) { return item.name; })
    };
  });

  data.releaseRecords = data.deployments.map(function (item, index) {
    return {
      orderNo: 'REL-20240220-' + pad2(index + 1),
      scriptName: item.scriptName,
      env: item.currentEnv,
      type: item.status === '回滚中' ? '回滚' : '变更',
      applicant: item.operator,
      reviewer: '系统复核',
      time: item.updatedAt,
      result: item.status === '回滚中' ? '回滚' : item.status === '已上线' ? '成功' : '处理中'
    };
  });

  data.grayAlerts = data.deployments
    .filter(function (item) { return item.currentEnv === 'GRAY' || item.status === '回滚中'; })
    .map(function (item) {
      return {
        level: item.status === '回滚中' ? '严重' : '警告',
        title: item.name + (item.status === '回滚中' ? ' 正在回滚' : ' 处于灰度验证阶段'),
        time: '09:30',
        status: item.status === '回滚中' ? '处理中' : '观察中'
      };
    });

  data.qualityRules = safeArray(data.qualityRules).map(function (item) {
    return {
      id: item.id,
      name: item.ruleName,
      type: item.ruleType,
      ...
      notify: item.alertThreshold || '失败立即告警',
      sqlCondition: item.sqlCondition,
      threshold: item.threshold,
      history: [{ time: formatDateTime(item.lastRunTime), passRate: Number(item.passRate || 0), failedCount: Number(item.passRate || 100) >= 100 ? 0 : 1, samples: [item.sqlCondition] }]
    };
  });

  data.batchJobs = safeArray(data.batchJobs).map(function (item) {
    var match = String(item.duration || '').match(/(\d+)/);
    var durationMinutes = match ? Number(match[1]) : 10;
    return {
      id: item.id,
      name: item.jobName,
      code: item.jobCode,
      cycle: item.schedule,
      status: normalizedJobStatus(item.status),
      lastRun: item.lastRunTime,
      nextRun: item.nextRunTime,
      duration: item.duration,
      dependency: safeArray(item.dependsOn).join('、') || '无',
      timelineStart: new Date(item.lastRunTime || '2024-02-20 00:00:00').getHours(),
      timelineDuration: durationMinutes,
      logs: [
        formatDateTime(item.lastRunTime) + ' INFO 作业启动',
        formatDateTime(item.lastRunTime) + ' INFO 目标表：' + safeArray(item.targetTables).join('、'),
        formatDateTime(item.lastRunTime) + ' INFO 阻断规则：' + safeArray(item.blockingRules).join('、')
      ],
      historyBars: [72, 76, 81, 78, 84, 86, 80],
      ioStats: {
        inputRows: '待补充',
        outputRows: '待补充',
        rejects: normalizedJobStatus(item.status) === '阻断中' ? '1' : '0'
      }
    };
  });

  data.blockingRules = data.qualityRules
    .filter(function (item) { return item.blockingLevel !== '警告'; })
    .map(function (item) {
      var impacted = data.batchJobs.filter(function (job) {
        return String(job.logs.join(' ')).indexOf(item.id) > -1;
      });
      return {
        triggerRule: item.id + ' ' + item.name,
        impactedJobs: impacted.length ? impacted.map(function (job) { return job.name; }).join('、') : '相关作业待补充',
        level: item.blockingLevel,
        notify: item.notify,
        recover: '修复数据后重新执行质量规则与跑批作业',
        enabled: item.enabled
      };
    });

  data.verificationAlerts = safeArray(data.notifications).map(function (item) {
    return {
      id: item.id,
      severity: item.type === 'error' ? '危急' : item.type === 'warning' ? '严重' : '提示',
      title: item.title,
      related: item.content,
      time: item.time,
      status: item.read ? '已解决' : '未处理',
      note: item.content
    };
  });

  var metadataBase = safeArray(data.metadataTables).map(function (item) {
    return {
      schema: item.schemaName || item.dbName,
      tableName: item.tableName,
      cnName: item.chineseName,
      type: item.tableType,
      owner: item.owner,
      sensitivity: sensitivityLevel(item.sensitivityLevel),
      rowCount: item.rowCount,
      storageSize: item.storageSize,
      partitions: item.partitionKey ? '1' : '0',
      columnCount: safeArray(item.columns).length,
      lastUpdated: item.lastScanned,
      tags: [item.tableType, item.partitionKey || '无分区'],
      fields: safeArray(item.columns).map(function (column) { return column.name; }),
      columns: safeArray(item.columns).map(function (column) {
        return {
          name: column.name,
          type: column.type,
          nullable: column.nullable ? '是' : '否',
          comment: column.comment,
          sample: safeArray(column.sampleValues)[0] || '',
          sensitivity: sensitivityLevel(item.sensitivityLevel)
        };
      }),
      sampleRows: [{ preview: item.description }],
      freshness: [90, 92, 93, 95, 96, 97, 98],
      trend: [12, 13, 14, 15, 16, 17, 18]
    };
  });

  data.metadataTables = metadataBase.map(function (item) {
    return {
      schema: item.schema,
      tableName: item.tableName,
  ...
  var catalogBySchema = {};
  metadataBase.forEach(function (table) {
    if (!catalogBySchema[table.schema]) {
      catalogBySchema[table.schema] = {
        key: table.schema,
        name: table.schema,
        label: table.schema.toUpperCase() + ' 层',
        color: '#2563eb',
        expanded: true,
        tables: []
      };
    }
    catalogBySchema[table.schema].tables.push({
      ...
      sampleRows: clone(table.sampleRows),
      freshness: clone(table.freshness),
      trend: clone(table.trend)
    });
  });
  data.metadataCatalog = Object.keys(catalogBySchema).map(function (key) { return catalogBySchema[key]; });

  data.scanConfigs = safeArray(data.scanConfigs).map(function (item) {
    return {
      name: item.name,
      db: item.targetDB,
      cycle: item.schedule,
      ...
      status: item.status === '警告' ? '运行中' : item.status,
      changes: item.changesFound,
      autoUpdate: !!item.autoUpdate,
      progress: item.status === '成功' ? 100 : 75
    };
  });

  data.metadataChanges = [
    { time: '2024-02-20 07:12', type: '新增字段', target: 'dw_ods.ods_cust_info.cust_segment', operator: '自动扫描', before: '-', after: 'varchar(20)' },
    { time: '2024-02-20 07:15', type: '刷新表结构', target: 'dw_dwd.dwd_cust_base_d', operator: '自动扫描', before: '旧版本', after: '新版本' },
    { time: '2024-02-20 07:18', type: '更新注释', target: 'dw_dws.dws_cust_asset_d.aum_amt', operator: '人工', before: 'AUM总量', after: 'AUM总量（含贵金属）' }
  ];

  var avgPassRate = data.qualityRules.length
    ? data.qualityRules.reduce(function (sum, item) { return sum + Number(item.passRate || 0); }, 0) / data.qualityRules.length
    : 0;
  data.qualityTrend = [0, 1, 2, 3, 4, 5, 6].map(function (offset) {
    return {
      date: 'D-' + (6 - offset),
      score: Math.max(90, Math.round(avgPassRate - 2 + offset * 0.4))
    };
  });
  data.metadataCoverage = data.metadataCatalog.length ? 96 : 0;
})(window.MockData);
